...
@property (atomic, MB_STRONG) NSMutableArray *loadingArray;
@end
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        if (!isActivityIndicator) {
            ...
            self.indicator = MB_AUTORELEASE([[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 100, 60)]);
            ...
            [_loadingArray addObject:[UIImage imageNamed:@"loadings0"]];
            [_loadingArray addObject:[UIImage imageNamed:@"loadings1"]];
            [_loadingArray addObject:[UIImage imageNamed:@"loadings2"]];
            //设置动画数组
            [(UIImageView *)self.indicator setAnimationImages:_loadingArray];
            //设置动画播放次数
            [(UIImageView *)self.indicator setAnimationRepeatCount:MAXFLOAT];
            //设置动画播放时间
            [(UIImageView *)self.indicator setAnimationDuration:0.5];
            //开始动画
