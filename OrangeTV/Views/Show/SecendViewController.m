...
-(void)setViews{
    ...
    if (!self.model) {
        ...
        [userDefa setValue:[NSNumber numberWithInteger:downloadState] forKey:@"downloadStateSecend"];
        [self.imgVideo setImage:[UIImage imageNamed:@"videoMaxCountLimit"]];
    }
    self.ProgressLabel.textColor = [UIColor whiteColor];
    //添加通知中心,监听下载进度
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downLoadOverAction:) name:@"secendDownLoadOver" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downLoadProgressAction:) name:@"secendDownLoadProgress" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(secendResourceNOAction) name:@"secendResourceNO" object:nil];
...
-(void)useFlowAction{
    ...
    if (!boolUseFlow) {
        if (![[URLRequestManager getNetWorkStates] isEqualToString:@"WIFI"]) {
            downloadStateType downloadState = [[userDefa valueForKey:@"downloadStateSecend"] integerValue];
        ...
        if (downloadState == pauseDownloading) {
            [userDefa setValue:[NSNumber numberWithInteger:isDownloading] forKey:@"downloadStateSecend"];
            [downloadMgr beginOrPauseDownload];
        }else if (downloadState == resourceIsNull)
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"secendVideoDeleted" object:nil];
        }
    }
}
...
-(void)dataBound{
    ...
    if (self.model) {
        ...
        if (self.model.sandboxFileName) {
            ...
            self.imgComment.hidden = NO;
            self.lblCommentCount.hidden = NO;
            downloadMgr = [[DownLoadSecend alloc]initWithUrl:[NSURL URLWithString:self.model.video]];
...
-(void)deleteVideo{
    ...
    if (dicCurrentVideoModel[@"2"]) {
        if (self.model.sandboxFileName) {
            ...
            [dicCurrentVideoModel setObject:[NSKeyedArchiver archivedDataWithRootObject:self.model] forKey:@"2"];
...
//        }
//        case isDownloading:{
//            self.downloadState = pauseDownloading;
//            [downloadMgr beginOrPauseDownload];
//            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                self.ProgressLabel.text = @"暂停下载";
//            });
...
//            self.downloadState = isDownloading;
//            [downloadMgr beginOrPauseDownload];
//            break;
//        }
//        case downloadOver:{
...
-(void)downLoadOverAction:(id)sender{
    ...
    NSUserDefaults *userDefa = [NSUserDefaults standardUserDefaults];
    [userDefa setValue:[NSNumber numberWithInteger:downloadOver] forKey:@"downloadStateSecend"];
    self.ProgressLabel.hidden = YES;
    self.myProgressView.hidden = YES;
    self.imgPlay.hidden = NO;
    ///将文件沙盒地址存入model
    ...
    [dicCurrentVideoModel setObject:modelData forKey:@"2"];
    [userInfo setObject:dicCurrentVideoModel forKey:@"dicCurrentVideoModel"];
}
...
-(void)secendResourceNOAction{
    NSUserDefaults *userDefa = [NSUserDefaults standardUserDefaults];
    [userDefa setValue:[NSNumber numberWithInteger:resourceIsNull] forKey:@"downloadStateSecend"];
    self.imgComment.hidden = YES;
    self.lblCommentCount.hidden = YES;
