...
@interface ShowCenterViewController ()
...
@implementation ShowCenterViewController
...
-(void)setViews{
    ...
    if (!self.model) {
        ...
        [userDefa setValue:[NSNumber numberWithInteger:downloadState] forKey:@"downloadState"];
        [self.imgVideo setImage:[UIImage imageNamed:@"videoMaxCountLimit"]];
    }
    self.ProgressLabel.textColor = [UIColor whiteColor];
    //添加通知中心,监听下载进度
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downLoadOverAction:) name:@"firstDownLoadOver" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downLoadProgressAction:) name:@"firstDownLoadProgress" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(firstResourceNOAction) name:@"firstResourceNO" object:nil];
...
-(void)dataBound{
    ...
    if (self.model) {
        ...
        if (self.model.sandboxFileName) {
            ...
            self.imgComment.hidden = NO;
            self.lblCommentCount.hidden = NO;
            downloadMgr = [[DownLoadManager alloc]initWithUrl:[NSURL URLWithString:self.model.video]];
...
-(void)deleteVideo{
    ...
    NSUserDefaults *userDefa = [NSUserDefaults standardUserDefaults];
    downloadStateType downloadState = [[userDefa valueForKey:@"downloadState"] integerValue];
    ...
    if (dicCurrentVideoModel[@"1"]) {
        if (self.model.sandboxFileName) {
            ...
            [dicCurrentVideoModel setObject:[NSKeyedArchiver archivedDataWithRootObject:self.model] forKey:@"1"];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"firstVideoDeleted" object:nil];
}
...
-(void)tabImgAction{
    ...
    switch (downloadState) {
        ...
        case pauseDownloading:{
            [userDefa setValue:[NSNumber numberWithInteger:isDownloading] forKey:@"downloadState"];
            [downloadMgr beginOrPauseDownload];
            break;
        }
        case downloadOver:{
...
-(void)downLoadOverAction:(id)sender{
    ...
    NSUserDefaults *userDefa = [NSUserDefaults standardUserDefaults];
    [userDefa setValue:[NSNumber numberWithInteger:downloadOver] forKey:@"downloadState"];
    self.ProgressLabel.hidden = YES;
    self.myProgressView.hidden = YES;
    self.imgPlay.hidden = NO;
    ///将文件沙盒地址存入model
    ...
    [dicCurrentVideoModel setObject:modelData forKey:@"1"];
    [userInfo setObject:dicCurrentVideoModel forKey:@"dicCurrentVideoModel"];
}

-(void)firstResourceNOAction{
    NSUserDefaults *userDefa = [NSUserDefaults standardUserDefaults];
    downloadStateType downloadState = resourceIsNull;
    [userDefa setValue:[NSNumber numberWithInteger:downloadState] forKey:@"downloadState"];
    self.imgComment.hidden = YES;
