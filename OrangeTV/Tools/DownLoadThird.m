...
@interface DownLoadThird ()<NSURLSessionDownloadDelegate>
{
    BOOL _isDownloading;
}
@end

@implementation DownLoadThird
//将session设置懒加载
...
didFinishDownloadingToURL:(NSURL *)location{
    ...
    //发送下载完成通知
    [[NSNotificationCenter defaultCenter] postNotificationName:@"thirdDownLoadOver" object:downloadTask.response.suggestedFilename];
}
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite{
    ...
    //发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:@"thirdDownLoadProgress" object:[NSNumber numberWithFloat:progress]];
}
